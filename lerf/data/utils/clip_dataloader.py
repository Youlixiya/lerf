...
class CLIPDataloader(FeatureDataloader):
    ...
    def create(self, image_list):
        clip_model = create_model(self.clip_model_type, pretrained=self.clip_pretrained, precision='fp16')
        ...
        for image in tqdm(image_list, desc="clip", total=len(image_list), leave=False):
            with torch.no_grad():
                image_tensor = preprocess(image)[None].cuda()
                clip_feature = clip_model.encode_image(image_tensor, 'ClearCLIP', True)
                feature_dim = clip_feature.shape[-1]
                h, w = image_tensor[0].shape[-2] // self.patch_size[0], image_tensor[0].shape[-1] // self.patch_size[1]
