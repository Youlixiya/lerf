...
class InputDataset(Dataset):
    ...
    def get_data(self, image_idx: int, image_type: Literal["uint8", "float32"] = "float32") -> Dict:
        ...
        # print(self._dataparser_outputs.object_mask_filenames)
        ...
        # if self._dataparser_outputs.object_mask_filenames is not None:
        image_filename = self._dataparser_outputs.image_filenames[image_idx]
        # object_mask_filename = str(image_filename).replace('images', 'object_mask').replace('jpg', 'png')
        # data["object_mask"] = torch.from_numpy(np.array(Image.open(object_mask_filename)))
