...
class ColmapDataParser(DataParser):
    ...
    def _get_all_images_and_cameras(self, recon_dir: Path):
        ...
        for im_id in ordered_im_id:
            ...
            if self.config.object_masks_path is not None:
                frame["object_mask_path"] = (
                    (self.config.data / self.config.object_masks_path / im_data.name).with_suffix(".png").as_posix()
                )
    ...
    def _generate_dataparser_outputs(self, split: str = "train", **kwargs):
        ...
        for frame in meta["frames"]:
            image_pil = Image.open(frame["file_path"])
            # image_pil
            ...
            height.append(image_pil.height)
            width.append(image_pil.width)
            distort.append(
            ...
            if 'mask_object_path' in frame:
                object_mask_filenames.append(Path(frame["mask_object_path"]))
        print(object_mask_filenames)
        ...
        image_filenames, mask_filenames, depth_filenames, downscale_factor = self._setup_downscale_factor(
            image_filenames, mask_filenames, depth_filenames, shape=(width[0], height[0])
        )
        ...
        dataparser_outputs = DataparserOutputs(
            ...
            object_mask_filenames=object_mask_filenames,
    ...
    def _setup_downscale_factor(
        self, image_filenames: List[Path], mask_filenames: List[Path], depth_filenames: List[Path], shape
    ):
        """
        ...
        if self._downscale_factor is None:
            if self.config.downscale_factor is None:
                ...
                test_img = test_img.resize(shape)
                w, h = test_img.size
                max_res = max(h, w)
                df = 0
                while True:
